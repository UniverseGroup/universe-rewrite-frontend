...
import {signJwt} from "../../../../tool/Tools";
...
export default async function handler(req, res) {
    ...
    const token = (await (await fetch(DiscordEnpoints.Token, {
        method: "POST",
        headers: {
    ...
    const user = (await (await fetch(DiscordEnpoints.Me, {
        method: 'GET',
        headers: {
    ...
    // res.setHeader(
    //     'set-cookie',
    //     cookie.serialize('token', signedJwt, {
    //         expires: new Date(userData.expires_in * 1000),
    //         secure: true,
    //         httpOnly: true,
    //         sameSite: 'lax',
    //         path: '/',
