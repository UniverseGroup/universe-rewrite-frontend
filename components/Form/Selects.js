...
const Select = ({ placeholder, options, values, setValues, handleChange, handleTouch }) => {
    ...
    return <SortableSelect useDragHandle axis='xy' distance={4} getHelperDimensions={({ node }) => node.getBoundingClientRect()} onSortEnd={onSortEnd}
        // select props
                           ...
                           }} isMulti className='border border-grey-light dark:border-transparent rounded' classNamePrefix='outline-none text-black cursor-pointer ' placeholder={placeholder || '선택해주세요.'} options={options} onChange={handleChange} onBlur={handleTouch} noOptionsMessage={() => '검색 결과가 없습니다.'}
